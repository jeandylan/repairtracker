...
app.controller('ticketEstimationCtrl',function ($scope,$location,editableOptions,$stateParams,serverServices,toaster,CacheFactory,ngDialog) {
    ...
    function getEstimates(){
        serverServices.get('api/estimation/'+1).then(function (result) {
    ...
    $scope.createEstimationLabour=function (labour) {
        serverServices.post('api/estimationLabour/'+$scope.ticketId,labour).then(function () {
    ...
    $scope.createEstimationStock=function (stock) {
        if(stock.qty_out) {
            serverServices.post('api/estimationItem/' + $scope.ticketId, stock).then(function () {
