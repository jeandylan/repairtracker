...
console.log($scope.employee);
        serverServices.post('api/allEmployee',$scope.employee) //using service (customer/service/clientService ) that will query Laravel for .json output
            .then(
                function (result) {
                    serverServices.post('api/allEmployeeAssignRole/'+result.id,{role:$scope.employee.role}).then(); //now we update The role by using The New Id obtain from server
                },
                function (error) {
                    // handle errors here
