...
app.controller('updateEmployeeCtrl', function($scope,serverServices,editableOptions,$filter, $state, $stateParams,toaster,$http) {
    ...
    $scope.updatePassword=function () {
        serverServices.put('api/allEmployeePassword/'+$scope.employeeId,{newPassword:$scope.newPassword}).then(function (result) {
            console.log(result)
            $scope.newPassword='';
        });
    };
    var domainName=location.host.split(".");
    var companyName = domainName[1];
    $scope.loadCompanyLocations = function() {
        serverServices.get('api/getActiveCompanyLocations/'+companyName).then(function (result) {
            $scope.shopLocations=result;
        });
    };
...
$scope.newPassword='';
    ...
    $scope.updateEmployee=function () { //when the submit btn click (function call found in <form> tag on top)
        ...
        serverServices.put('api/allEmployee/'+$scope.employeeId,$scope.employee).then(function (result) {
            console.log(result);
        })
    };
    ...
    function  getEmployeeData() {
        serverServices.get('api/allEmployee/'+$scope.employeeId)//id parameter obtain by doing state parameter (like a query)
            .then(
                function (result) {
    ...
    $scope.updateEmployeeRole=function () {
        serverServices.post('api/allEmployeeAssignRole/'+$scope.employeeId,{role:$scope.employee.role}).then(function () {
